...
def on_drag_motion(event):
    """
    Moves the selected canvas window item smoothly without leaving behind
    any residual artifacts. We also lift the item and force a redraw so
    that the canvas clears the previous footprint before drawing the widget
    in its new position.
    """
    ...
    # Move the window‐item to its new coordinates
    content_canvas.coords(drag_data["win_id"], new_x, new_y)
    # Bring it to the front so the canvas knows to redraw that area
    content_canvas.lift(drag_data["win_id"])
    # Update the scroll region if you’ve dragged past the edge
    update_scrollregion()
    # Force the canvas to process all pending redraws—this clears any trails
    content_canvas.update_idletasks()


def on_drag_release(event):
    """
    When the mouse button is released, finalize the move by calculating
    the 'base' (unzoomed) coordinates, saving them in the DB, and then
    forcing a final redraw so nothing gets left behind.
    """
    win_id = drag_data["win_id"]
    if win_id is None:
        return

    # Get the final canvas coords, then convert back to unscaled values
    ...
    # Final refresh to erase any leftover artifacts
    update_scrollregion()
    content_canvas.update_idletasks()
